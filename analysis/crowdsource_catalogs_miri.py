...
def main(bg_boxsizes={'f2550w':11}, # need to confirm this using BackgroundEstimationExperiments.ipynb
         crowdsource_default_kwargs={'maxstars': 5000, },
        ):
    ...
    parser.add_option("-i", "--field", dest="field",
                    default='001', # Note: for 2221, the target is 'cloudc', not 'brick' for field 001
    ...
    parser.add_option("--target", dest="target",
                    default='cloudc', # Note: for 2221, the target is 'cloudc', not 'brick' for field 001
    ...
    modules = '' # no modules for MIRI
    pupil = '' # no pupils for MIRI
    ...
    for module in modules:
        detector = '' # no detectors for MIRI - they're all the same
...
def get_filename(basepath, filtername, proposal_id, field, module, options, pupil='clear'):
    ...
    if not os.path.exists(filename):
        ...
        filename = f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}_i2d{desat}.fits'
    if not os.path.exists(filename):
        pupil = 'F444W'
        filename = f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}_nodestreak_realigned-to-refcat.fits'
    if not os.path.exists(filename):
        filename = f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}_realigned-to-refcat.fits'
    if not os.path.exists(filename):
        # merged-reproject_i2d.fits lives here
        ...
        filename = f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}_i2d{desat}.fits'
    if not os.path.exists(filename):
        glstr = f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_*-{module}_realigned-to-refcat.fits'
        fglob = glob.glob(glstr)
        if len(fglob) == 1:
            filename = fglob[0]
        else:
...
def do_photometry_step(options, filtername, module, detector, field, basepath,
    ...
    try:
        ...
        pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}_catalog_diagnostics_daofind.png',
                bbox_inches='tight')
        ...
        pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}_catalog_diagnostics_zoom_daofind.png',
                bbox_inches='tight')
        ...
        for name, zoomcut in zoomcut_list.items():
            ...
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}_catalog_diagnostics_zoom{name.replace(" ","_")}_daofind.png',
                    bbox_inches='tight')
    except Exception as ex:
    ...
    if not options.nocrowdsource:
        ...
        try:
            ...
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{blur_}_catalog_diagnostics_unweighted.png',
                    bbox_inches='tight')

            catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
            pl.suptitle(f"Crowdsource nsky=1 unweighted Catalog Diagnostics {filtername} {module}{exposure_}{desat}{bgsub}{blur_}")
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{blur_}_catalog_diagnostics_zoom_unweighted.png',
                    bbox_inches='tight')
            for name, zoomcut in zoomcut_list.items():
                catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
                pl.suptitle(f"Crowdsource nsky=1 Catalog Diagnostics {filtername} {module}{exposure_}{desat}{bgsub}{blur_} zoom {name}")
                pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{blur_}_catalog_diagnostics_zoom{name.replace(" ","_")}_unweighted.png',
                        bbox_inches='tight')
        except Exception as ex:
        ...
        pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}_weights.png',
                   bbox_inches='tight')
        ...
        for refit_psf, fpsf in zip((False, True), ('', '_fitpsf',)):
            for nsky in (0, 1, ):
                ...
                try:
                    ...
                    pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{fpsf}{blur_}_nsky{nsky}_weighted_catalog_diagnostics.png',
                            bbox_inches='tight')

                    catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
                    pl.suptitle(f"Catalog Diagnostics zoomed {filtername} {module}{exposure_}{desat}{bgsub}{fpsf}{blur_} nsky={nsky} weighted")
                    pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{fpsf}{blur_}_nsky{nsky}_weighted_catalog_diagnostics_zoom.png',
                            bbox_inches='tight')
                    ...
                    for name, zoomcut in zoomcut_list.items():
                        ...
                        pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{fpsf}{blur_}_nsky{nsky}_weighted_catalog_diagnostics_zoom{name.replace(" ","_")}.png',
                                bbox_inches='tight')
                except Exception as ex:
    ...
    if options.daophot:
        ...
        if options.epsf:
            ...
            pl.colorbar()
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_daophot_epsf.png',
                       bbox_inches='tight')
            dao_psf_model = epsf

            save_epsf(epsf,
                      f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_daophot_epsf.fits')
        ...
        fits.PrimaryHDU(data=residual, header=im1[1].header).writeto(
            f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_daophot_basic_residual.fits',
            overwrite=True)
        fits.PrimaryHDU(data=modsky, header=im1[1].header).writeto(
            f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_daophot_basic_model.fits',
            overwrite=True)
        ...
        try:
            ...
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_catalog_diagnostics_daophot_basic.png',
                    bbox_inches='tight')

            catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
            pl.suptitle(f"daophot basic Catalog Diagnostics {filtername} {module}{exposure_}{desat}{bgsub}{epsf_}{blur_}")
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_catalog_diagnostics_zoom_daophot_basic.png',
                    bbox_inches='tight')

            for name, zoomcut in zoomcut_list.items():
                catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
                pl.suptitle(f"daophot basic Catalog Diagnostics {filtername} {module}{exposure_}{desat}{bgsub}{epsf_}{blur_} zoom {name}")
                pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}__catalog_diagnostics_zoom_daophot_basic{name.replace(" ","_")}.png',
                        bbox_inches='tight')
        except Exception as ex:
    ...
    if options.daophot:
        ...
        fits.PrimaryHDU(data=residual, header=im1[1].header).writeto(
            f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_daophot_iterative_residual.fits',
            overwrite=True)
        fits.PrimaryHDU(data=modsky, header=im1[1].header).writeto(
            f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_daophot_iterative_model.fits',
            overwrite=True)
        ...
        try:
            ...
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_catalog_diagnostics_daophot_iterative.png',
                    bbox_inches='tight')

            catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
            pl.suptitle(f"daophot iterative Catalog Diagnostics {filtername} {module}{exposure_}{desat}{bgsub}{epsf_}{blur_}")
            pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}_catalog_diagnostics_zoom_daophot_iterative.png',
                    bbox_inches='tight')

            for name, zoomcut in zoomcut_list.items():
                catalog_zoom_diagnostic(data, modsky, zoomcut, stars)
                pl.suptitle(f"daophot iterative Catalog Diagnostics {filtername} {module}{exposure_}{desat}{bgsub}{epsf_}{blur_} zoom {name}")
                pl.savefig(f'{basepath}/{filtername}/pipeline/jw0{proposal_id}-o{field}_t001_miri_{filtername.lower()}{exposure_}{desat}{bgsub}{epsf_}{blur_}__catalog_diagnostics_zoom_daophot_iterative{name.replace(" ","_")}.png',
                        bbox_inches='tight')
        except Exception as ex:
